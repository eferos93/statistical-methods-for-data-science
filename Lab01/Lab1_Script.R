...
knitr::opts_chunk$set(fig.align = 'center', warning=FALSE, message=FALSE,
                      fig.asp=0.625, dev='png', global.par = TRUE, 
                      dev.args=list(pointsize=10), fig.path = 'figs/'
                     )
library(MASS)
...
plot(year, carbon, pch=3, col="red")
...
plot(0:1000, dnbinom(0:1000, size=r, prob=p), xlab="k",
...
# 1 means sum by row (each i-th value of Y is the sum of the i-th row of X)
Y<-apply(X,1,sum)
...
t_mixture <- function(mean,  df, n){
  ...
  Y=rgamma(n, df/2, df/2)
  X=rnorm(n, mean, sd=sqrt(1/sqrt(Y)))
  return(X)
